...
# cascade_path = "C:\opencv\sources\data\lbpcascades\lbpcascade_frontalface.xml"

cnt = 0
cascade_list = ["C:\\opencv\\sources\data\\haarcascades\\haarcascade_frontalface_alt.xml",
...
# カスケード分類器の特徴量を取得する
for cascade_path in cascade_list:
    cnt += 1
    print(cascade_path)
    ...
    if len(facerect) > 0:
        ...
        # 認識結果の保存
        cv2.imwrite("detected" + str(cnt) + ".jpg", image)